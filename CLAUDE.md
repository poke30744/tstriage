...
5. **mark**: `tsmarker mark` (subtitles/clipinfo/logo/speech/ensemble) + whisper STT → `.generated.srt` → LLM correction → `.corrected.srt` → gap fix (end-time correction for VAD bugs) + EDL; create `.tocut`
